...
```{r}
#shiny app plus tab it gives it fills it
```
...
```{r, eval=F, echo=T}
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm") # selected refers to drop menu
)
...
2. for y. mostly coping and pasting and changing to y
```{r, eval=F, echo=T}
ui <- fluidPage(
...
shinyApp(ui, server)
```
This are just strings they are not attached to any data this is just the user input.
...
ui <- fluidPage(
  radioButtons("x", "Select Fill Variable", choices = c( "tophic.guild", "thermalregulation"), 
...
server <- function(input, output, session) {
  output$plot<-renderPlot({
    ggplot(data=homerange,aes_string(x="locomotion", fill=input$x))+
      geom_bar(position="dodge", alpha=0.8, color= "blue")+
