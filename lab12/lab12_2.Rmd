...
ui <- fluidPage(titlePanel("Log 10 Homerange by Taxon"),
                selectInput("x", "Select Taxon",
                             choices=c("birds", "mammals","lake fishes", "lizards","marine fishes", "river fishes", "snakes",  "tortoises", "turtles"), selected="birds"),
                plotOutput("plot", width="600px", height="500px")
)
  ...
  output$plot <- renderPlot({
   homerange %>% 
     filter(taxon==input$x) %>% 
      ...
      geom_densiy(fill="steelbue", color="black", alpha=0.6)
  })
