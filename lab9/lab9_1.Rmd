...
```{r}
is.na(homerange)
# produces a long messy revelation of each NA in the data
```
...
```{r}
ggplot(data=homerange,mapping=aes(x=log10.hra, y=log10.preymass))+geom_point()# ggplot remove NA in the data our pllot basically deleted them and did nto plot them 
```
...
```
```{r}
# different code used by professor
```
