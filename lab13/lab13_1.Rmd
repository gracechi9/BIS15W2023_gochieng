...
```{r, message = FALSE}
spider_list <- lapply(files, read_csv) # lappply makes a list and go into the file and read csv
...
inner_exampleDF
# refers to joining between(only common data)
```
...
```{r}
dmy(spiders_with_locs$Date) #day month year but this is not clean
```
