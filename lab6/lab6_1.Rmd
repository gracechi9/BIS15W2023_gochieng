...
```{r}
# Na usually means inpouts that were not found/ collected and summarize tends to count for it as well (unless we specifically tell R to identify it and remove it)
```
...
msleep %>% 
  ...
  select(brainwt)
# unfortunately this order had no data inoutted thus we see the values missing
```
